...
function commitMessages(options) {
    ...
    request({uri: url, json: true, headers: {'User-Agent': userAgent}}, function (err, res, data) {
        ...
        var prefixes = "(feature),(fix),(clean)"
        var searches = prefixes.split(",")
        changes = _.sortBy(changes, function(change){
            // console.log(change)
            var sortIdx = searches.length
            _.forEach(searches,function(search, idx){
                if (change.message.indexOf(search) != -1) {sortIdx = idx;return false};
            })
            return sortIdx
        })
